...
assert.equal(button.attrs['aria-label'], 'Open menu');
const open = () => { button.handlers.click(); assert.equal(button.attrs['aria-expanded'], 'true'); assert.equal(button.attrs['aria-label'], 'Close menu'); };
const closed = () => { assert.equal(button.attrs['aria-expanded'], 'false'); assert.equal(button.attrs['aria-label'], 'Open menu'); assert(!classes.has('menu-open')); };
